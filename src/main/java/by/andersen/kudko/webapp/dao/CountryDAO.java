...
public class CountryDAO extends AbstractDAO<Country, Integer> {
    public final String INSERT_USER = "INSERT INTO `travel_agency`.`country` (`name`) VALUES (?);";
    public final String FIND_BY_PK = "SELECT `id`,`name` FROM `travel_agency`.`country` u WHERE u.id = ?;";
    public final String UPDATE_BY_ID = " UPDATE `travel_agency`.`country` SET `name` = ? WHERE `id` = ?;";
    ...
    @Override
    public Country resultsetStringToObject(ResultSet resultSet) throws SQLException {
